...
public class SecurityConfig {
	...
	public InMemoryUserDetailsManager inMemoryUserDetailsManager() {
		return new InMemoryUserDetailsManager(
				...
				User.withUsername("user1").password(passwordEncoder.encode("****")).roles("USER").build(),
				User.withUsername("user2").password(passwordEncoder.encode("****")).roles("USER").build()
				);
	}
	@Bean
...
//		1er méthode pour protege les resources;
